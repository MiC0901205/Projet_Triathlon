...
go
--alter role db_datareader add MEMBER EntityCinema;
--alter role db_datawriter add MEMBER EntityCinema;
--go
...
    @numTriathlon int
    AS
        SELECT T.numLicence, nom, prenom, tempsCourse, tempsCyclisme, tempsNatation, SUM(tempsCourse + tempsCyclisme + tempsNatation) as tempsTotal
        FROM Triathlete T
            JOIN Inscription I on I.numLicence = T.numLicence
        WHERE numTriath = @numTriathlon
        GROUP BY T.numLicence, nom, prenom, tempsCourse, tempsCyclisme, tempsNatation
        ORDER BY tempsTotal;
go
